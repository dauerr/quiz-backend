...
export const like = async (event: APIGatewayEvent, context: Context) => {
    const email = event.requestContext?.authorizer?.claims.email;
    if (!email) {
        return {
            statusCode: 400,
...
export const getLikes = async (event: APIGatewayEvent, context: Context) => {
    const quizParam = event.queryStringParameters?.quizId;
    if (!quizParam) {
        return {
            statusCode: 500,
    ...
    const quizId = parseInt(quizParam);
    const db = new Database();
    const likes = await db.getLikes(quizId);
    await db.shutdown();
    return {
        statusCode: 200,
        body: likes,
...
export const dislike = async (event: APIGatewayEvent, context: Context) => {
    const email = event.requestContext?.authorizer?.claims.email;
    if (!email) {
        return {
            statusCode: 400,
    ...
    const db = new Database();
    const userId = await db.getUserIdForEmail(email);
    await db.dislike(userId, quizId);
    await db.shutdown();
    return {
        statusCode: 200,
        body: "OK",
...
export const getDislikes = async (event: APIGatewayEvent, context: Context) => {
    const quizParam = event.queryStringParameters?.quizId;
    if (!quizParam) {
        return {
            statusCode: 500,
    ...
    const quizId = parseInt(quizParam);
    const db = new Database();
    const likes = await db.getDislikes(quizId);
    await db.shutdown();
    return {
        statusCode: 200,
        body: likes,
